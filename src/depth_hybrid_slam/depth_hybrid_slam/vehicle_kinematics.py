"""ROS-independent Ackermann kinematics used for planner regression tests."""
...
class AckermannPathEvaluator:
    """Deterministic bicycle model with measured T870 geometry."""
    ...
    def __init__(self, stage_speeds_mps, reverse_speed_mps,
        ...
        if (abs(self.wheelbase-WHEELBASE_M) > 1.0e-9 or
                ...
                self.encoder_signed):
            raise ValueError("kinematics must use measured calibration")
    ...
    def step(self, drive_stage, steer_deg, stop, dt_s):
        ...
        if not math.isfinite(dt) or dt <= 0.0 or dt > 2.0:
            raise ValueError("integration dt must be in (0, 2]")
